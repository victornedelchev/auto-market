 ...
 * Features caching, request deduplication, and rate limiting.
 */

const API_URL = import.meta.env.VITE_AI_API_URL || 'https://api.openai.com/v1/chat/completions';
const API_KEY = import.meta.env.VITE_AI_API_KEY;

// --- State Management for AI Requests ---
const responseCache = new Map();
const pendingRequests = new Map();
let lastRequestTime = 0;
const MIN_REQUEST_INTERVAL_MS = 1000; // Rate limit: minimum 1s between requests
...
/**
 * Core function to handle API calls to the AI service.
 * Handles caching, deduplication, and rate limiting.
 * @param {string} prompt - The prompt to send to the AI.
 * @param {Object} [options] - Options like temperature or max_tokens.
 * @returns {Promise<string>} The generated text.
 * @throws {Error} If the network request fails or API returns an error.
 */
async function callAI(prompt, options = {}) {
    if (!API_KEY) {
        throw new Error('AI service is currently unavailable: API key is not configured.');
    }

    // 1. Caching: Return cached response if available
    if (responseCache.has(prompt)) {
        return responseCache.get(prompt);
    }

    // 2. Deduplication: Return pending promise if request is already inflight
    if (pendingRequests.has(prompt)) {
        return pendingRequests.get(prompt);
    }

    // 3. Centralized API Call Logic
    const makeRequest = async () => {
        ...
        try {
            ...
            responseCache.set(prompt, resultText);
            return resultText;
            ...
            // Clean up the pending request lock
            pendingRequests.delete(prompt);
        }
    };

    // Store the promise in pendingRequests to deduplicate identical concurrent calls
 ...
 * @returns {Promise<string>} The generated description in plain text, or an error message.
 */
...
Color: ${carData.color || 'Unknown'}`;
    ...
    try {
        return await callAI(prompt, { temperature: 0.7, max_tokens: 500 });
    } catch (error) {
        return error.message || 'We encountered a network error while connecting to the AI service. Please write your own description.';
    }
}
 ...
 * @returns {Promise<string>} The improved description in plain text, or an error message.
 */
...
${existingDescription}`;
    ...
    try {
        ...
        return error.message || 'We encountered a network error while connecting to the AI service. Please try again.';
    }
}
 ...
 * @returns {Promise<string>} The generated title in plain text, or an error message.
 */
...
Mileage: ${carData.mileage || 'Unknown'} km`;
    ...
    try {
        let title = await callAI(prompt, { temperature: 0.7, max_tokens: 50 });
        ...
        return error.message || 'AI Network Error';
    }
}
...
${text}`;
    ...
    try {
        let resultText = await callAI(prompt, { temperature: 0.1, max_tokens: 500 });
...
Description: ${carData.description ? carData.description.substring(0, 500) : 'None'}`;
    ...
    try {
        return await callAI(prompt, { temperature: 0.5, max_tokens: 100 });
    } catch (error) {
...
Description: ${car2.description || 'None'}`;
    ...
    try {
        let resultText = await callAI(prompt, { temperature: 0.7, max_tokens: 1500 });
        ...
        return `<div class="alert alert-danger">${error.message || 'Network error while connecting to AI.'}</div>`;
    }
}
